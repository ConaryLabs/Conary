// crates/conary-core/src/federation_discovery.rs
...
impl std::fmt::Display for PeerTier {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::RegionHub => write!(f, "region_hub"),
            Self::CellHub => write!(f, "cell_hub"),
            Self::Leaf => write!(f, "leaf"),
        }
    }
}

impl std::str::FromStr for PeerTier {
    type Err = &'static str;

    fn from_str(value: &str) -> std::result::Result<Self, Self::Err> {
        match value {
            "region_hub" => Ok(Self::RegionHub),
            "cell_hub" => Ok(Self::CellHub),
            "leaf" => Ok(Self::Leaf),
            _ => Err("invalid peer tier"),
        }
    }
}
    ...
    match event {
        ServiceEvent::ServiceResolved(info) => {
            ...
            let node_id = properties
                .get("node_id")
                .cloned()
                .unwrap_or_else(|| hash::sha256(info.get_fullname().as_bytes()));
        ...
        ServiceEvent::ServiceRemoved(_, fullname) => {
            ...
            let peer_id = hash::sha256(fullname.as_bytes());
...
mod tests {
    ...
    #[test]
    fn test_discovered_peer_builds_http_endpoint() {
        ...
        let endpoint = discovered.endpoint_with_secure_transport(false).unwrap();
        assert_eq!(endpoint, "http://192.168.1.100:7891");
    }

    #[test]
    fn test_discovered_peer_endpoint_prefers_ipv4() {
        let discovered = DiscoveredPeer {
            id: "test-id".to_string(),
            instance_name: "test-instance".to_string(),
            hostname: "test-host.local.".to_string(),
