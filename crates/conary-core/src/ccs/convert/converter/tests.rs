...
#[test]
fn conversion_metrics_expose_each_chunked_payload_pass_and_temporary_durability() {
    let temp_dir = tempfile::tempdir().unwrap();
    let metadata = make_test_metadata();
    let converter = passive_test_converter(temp_dir.path());
    ...
    assert_eq!(
        ccs_write.temporary_object_file_syncs,
        metrics.payload_chunks_derived
    );
    assert_eq!(
        ccs_write.temporary_object_shard_syncs,
        ccs_write.temporary_object_misses
    );
