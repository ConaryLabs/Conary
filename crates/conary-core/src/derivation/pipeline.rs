...
use super::install::install_to_sysroot;
...
impl Pipeline {
        ...
        F: FnMut(&PipelineEvent),
    {
        ...
        if !acc.manifests.is_empty() {
            ...
            let root = crate::generation::root_manifest::capture_root_node(&sysroot)
                .map_err(|error| PipelineError::Io(error.to_string()))?;

            let build_result = compose_erofs(&manifest_refs, root, &compose_dir)?;
...
fn load_manifest_from_cas(
    ...
    let manifest = OutputManifest::from_toml(toml_str)
