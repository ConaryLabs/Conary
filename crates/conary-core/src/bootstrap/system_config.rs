...
fn detect_boot_kernel(root: &Path) -> Result<std::path::PathBuf, SystemConfigError> {
    ...
    for entry in entries {
        let entry = entry.map_err(|error| {
            SystemConfigError::ConfigFailed(format!(
                "cannot inspect boot kernel candidate: {error}"
            ))
        })?;
        let path = entry.path();
    ...
    match versioned.as_slice() {
        ...
        [] => Err(SystemConfigError::ConfigFailed(
            "kernel image not found under /boot (expected one exact vmlinuz-<release> from Phase 3)"
                .into(),
        )),
        _ => Err(SystemConfigError::ConfigFailed(format!(
            "multiple versioned kernels under /boot are ambiguous: {}; Phase 3 must select one exact release",
            versioned
                .iter()
