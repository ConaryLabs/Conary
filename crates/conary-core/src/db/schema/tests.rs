...
#[test]
fn revision_45_requires_rebuild_for_revision_48() {
    let conn = Connection::open_in_memory().unwrap();
    conn.execute_batch(
    ...
    assert_eq!(
        error.to_string(),
        "Database schema rebuild required: database uses schema epoch conary-current-v1 revision 45; this pre-alpha build supports only schema epoch conary-current-v1 revision 48"
    );
}
