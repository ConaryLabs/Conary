...
impl Conflict {
    ...
    pub fn graph<D: DependencyProvider, RT: AsyncRuntime>(
        ...
        root_reachable_conflict_graph(graph, root_node, unresolved_node)
    }
...
/// Retain the causal proof rooted at the user's requirements.
///
/// Conflict analysis can retain a learned clause for a candidate that was
/// explored and rejected before the final root conflict was established. That
/// clause is valid solver evidence, but it has no incoming path from the root
/// in the final diagnostic graph. It must not make rendering panic or appear
/// as an unrelated top-level explanation.
fn root_reachable_conflict_graph<S: SolverId>(
    graph: DiGraph<ConflictNode<S>, ConflictEdge<S>>,
    root_node: NodeIndex,
    ...
    if graph.node_count() == reachable.len() {
    ...
    for old in graph.node_indices().filter(|node| reachable.contains(node)) {
        let new = filtered.add_node(*graph.node_weight(old).expect("conflict node exists"));
        remapped.insert(old, new);
    }
    for edge in graph.edge_references() {
        let (Some(&source), Some(&target)) =
            (remapped.get(&edge.source()), remapped.get(&edge.target()))
        else {
            continue;
        };
        filtered.add_edge(source, target, *edge.weight());
    }

    ConflictGraph {
        graph: filtered,
        root_node: remapped[&root_node],
        unresolved_node: unresolved_node.and_then(|node| remapped.get(&node).copied()),
    }
}
...
mod tests {
    ...
    fn conflict_graph_discards_learned_branches_unreachable_from_root() {
        let mut graph = DiGraph::<ConflictNode, ConflictEdge>::new();
        let root = graph.add_node(ConflictNode::Root);
        let causal = graph.add_node(ConflictNode::Solvable(SolvableId::from_raw(0)));
        let unresolved = graph.add_node(ConflictNode::UnresolvedDependency);
        let unrelated = graph.add_node(ConflictNode::Solvable(SolvableId::from_raw(1)));
        let requirement = ConflictEdge::Requires(Requirement::Single(VersionSetId(0)));
        graph.add_edge(root, causal, requirement);
        graph.add_edge(causal, unresolved, requirement);
        graph.add_edge(unrelated, unresolved, requirement);

        let filtered = root_reachable_conflict_graph(graph, root, Some(unresolved));
        ...
        assert!(!filtered.graph.node_weights().any(
            |node| matches!(node, ConflictNode::Solvable(id) if *id == SolvableId::from_raw(1))
        ));
    }
}
