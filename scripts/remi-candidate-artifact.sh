...
usage() {
    cat >&2 <<'EOF'
Usage:
  scripts/remi-candidate-artifact.sh package BINARY OUTPUT_DIR VERSION COMMIT RUN_ID SETUP_MS CACHE_SETUP_MS BUILD_MS LINK_TIMINGS SCCACHE_STATS
...
package_artifact() {
    ...
    local build_ms="$8"
    local link_timings="$9"
    local sccache_stats="${10}"
    ...
    local retained_link_timings retained_sccache_stats
    ...
    [[ ! -e "$artifact_path" && ! -e "$bundle_path" && ! -e "$manifest_path" \
        && ! -e "$retained_link_timings" && ! -e "$retained_sccache_stats" ]] ||
    ...
    local linker_path linker_sha
    linker_path="$(realpath scripts/timed-linker.sh)"
    linker_sha="$(sha256_file "$linker_path")"
    local link_timings_sha link_timings_bytes sccache_stats_sha sccache_stats_bytes
...
verify_artifact() {
    ...
    local binary_name bundle_name binary bundle link_timings sccache_stats
