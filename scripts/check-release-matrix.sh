...
require_match "$resolution_survey_transport" 'validate_comparison_survey\([\s\S]*candidate_roots_walked[\s\S]*roots != candidate_roots_walked[\s\S]*candidate_roots\.get\(root_sha256\)[\s\S]*candidate outcome differs from its survey[\s\S]*retained mismatch roots are noncanonical' 'resolution survey comparison binds the exact candidate root population'
