...
require_match "$native_oracle_production_workflow" 'workflow_dispatch:[\s\S]*export_run_id:[\s\S]*required: true[\s\S]*producer_commit:[\s\S]*use the deployed commit by default[\s\S]*required: true[\s\S]*permissions:[\s\S]*actions: read[\s\S]*contents: read[\s\S]*cancel-in-progress: false' 'native-oracle production exact export and producer inputs with read-only permissions'
require_job_match "$native_oracle_production_workflow" authorize 'timeout-minutes: 20[\s\S]*environment: production[\s\S]*PRODUCER_COMMIT[\s\S]*\^\[0-9a-f\]\{40\}\$[\s\S]*GITHUB_REF" == refs/heads/main[\s\S]*git fetch origin main[\s\S]*git merge-base --is-ancestor HEAD origin/main' 'native-oracle production protected-main and full producer SHA authorization'
...
require_job_match "$native_oracle_production_workflow" authorize 'git merge-base --is-ancestor "\$DEPLOYED_COMMIT" "\$PRODUCER_COMMIT"[\s\S]*git merge-base --is-ancestor "\$PRODUCER_COMMIT" origin/main[\s\S]*producer_commit=\$PRODUCER_COMMIT' 'native-oracle production merged descendant producer predicate'
...
require_job_match "$native_oracle_production_workflow" produce 'ref: \$\{\{ needs\.authorize\.outputs\.producer_commit \}\}[\s\S]*git rev-parse HEAD[\s\S]*== "\$PRODUCER_COMMIT"[\s\S]*git status --porcelain[\s\S]*cargo build --release -p conary-core[\s\S]*produce-native-oracle-lane\.py[\s\S]*--producer-commit "\$PRODUCER_COMMIT"' 'native-oracle production exact clean producer source and typed lane adapter'
...
require_job_match "$native_oracle_production_workflow" produce '\.schema_version == 2[\s\S]*\.package_oracle\.schema_version == 1[\s\S]*\.resolution_oracle\.schema_version == 2[\s\S]*projection_schema:4[\s\S]*projection_schema:2' 'native-oracle production exact lane, package, resolution, and implementation schemas'
...
require_match "$native_oracle_lane_producer" 'producer_binary[\s\S]*must be a regular file, never a symlink[\s\S]*sha256_file[\s\S]*"schema_version": 2[\s\S]*"producer_commit"[\s\S]*"producer_binaries"' 'native-oracle lane exact producer commit and binary digest binding'
