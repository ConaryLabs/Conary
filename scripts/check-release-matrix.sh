...
immutable_publish_pattern='if gh release view "\$TAG_NAME" >/dev/null 2>&1; then[\s\S]*--json isDraft --jq[\s\S]*release \$TAG_NAME is already published; refusing to replace immutable assets[\s\S]*else[\s\S]*gh release create "\$TAG_NAME"[\s\S]*--draft[\s\S]*--generate-notes[\s\S]*--verify-tag[\s\S]*fi[\s\S]*gh release edit "\$TAG_NAME" --notes-file "\$release_notes"[\s\S]*gh release upload "\$TAG_NAME" suite-packages/\* --clobber[\s\S]*diff -u "\$local_names" "\$remote_names"[\s\S]*draft release digest[\s\S]*gh release edit "\$TAG_NAME" --draft=false'
