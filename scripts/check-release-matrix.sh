...
require_job_match "$nightly_release" select-green-main 'actions/workflows/merge-validation\.yml/runs[\s\S]*branch=main[\s\S]*status=success[\s\S]*git checkout --detach "\$commit_sha"[\s\S]*nightly-release\.py preflight --commit "\$commit_sha" --workflow-commit "\$WORKFLOW_COMMIT"' 'nightly newest green-main selection and capability preflight'
