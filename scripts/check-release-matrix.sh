...
require_job_match "$native_oracle_production_workflow" produce '\.schema_version == 2[\s\S]*\.artifact_type == "native-resolution-survey-diagnostics"[\s\S]*\.survey\.schema_version == 3[\s\S]*\.schema_version == 4[\s\S]*\.artifact_type == "native-oracle-lane"[\s\S]*\.package_oracle\.schema_version == 1[\s\S]*\.resolution_oracle\.schema_version == 3[\s\S]*projection_schema:5[\s\S]*projection_schema:3' 'native-oracle production exact lane, package, resolution, and implementation schemas'
...
require_match "$native_oracle_lane_producer" 'NATIVE_RESOLUTION_SURVEY_SCHEMA = 3[\s\S]*--survey[\s\S]*write_resolution_survey[\s\S]*native-resolution-survey-diagnostics[\s\S]*invoke\(resolution_command, "native resolution producer"\)' 'native-oracle lane writes and validates diagnostics survey before strict resolution'
