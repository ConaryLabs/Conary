...
require_job_match "$native_oracle_production_workflow" produce '\.schema_version == 2[\s\S]*\.artifact_type == "native-resolution-survey-diagnostics"[\s\S]*\.schema_version == 4[\s\S]*\.artifact_type == "native-oracle-lane"[\s\S]*\.package_oracle\.schema_version == 1[\s\S]*\.resolution_oracle\.schema_version == 2[\s\S]*projection_schema:4[\s\S]*projection_schema:2' 'native-oracle production exact lane, package, resolution, and implementation schemas'
...
require_match "$resolution_survey_transport" 'validate_lane\([\s\S]*resolution_implementation[\s\S]*validate_resolution_implementation[\s\S]*schema_version[^\n]*!= 4[\s\S]*native-oracle-lane[\s\S]*producer_commit[\s\S]*lane_evidence_sha256[\s\S]*authenticated three-lane assembly[\s\S]*resolution oracle is not bound to its package oracle' 'resolution survey validator exact assembled oracle lane bindings'
require_match "$resolution_survey_transport" 'reject_duplicate_key[\s\S]*tarfile\.open\(args\.transport, mode="r:"\)[\s\S]*survey transport repeats member[\s\S]*canonical_json\(manifest\) != manifest_bytes[\s\S]*exact_u32\(manifest\["schema_version"\][\s\S]*copy_declared_survey_member[\s\S]*forbid_private_path_bytes' 'resolution survey strict sanitized output transport verification'
...
require_match "$remi_deploy_helper" 'candidate-resolution-implementation\.json[\s\S]*comparison-resolution-implementation\.json[\s\S]*implementation_file: \$candidate_implementation_file[\s\S]*implementation_file: \$comparison_implementation_file[\s\S]*schema_version: 2' 'resolution survey helper retains separately bound worker implementation evidence'
