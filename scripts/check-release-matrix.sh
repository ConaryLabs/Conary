...
require_job_match "$native_oracle_production_workflow" authorize 'timeout-minutes: 20[\s\S]*environment: production[\s\S]*PRODUCER_COMMIT[\s\S]*\^\[0-9a-f\]\{40\}\$[\s\S]*GITHUB_REF" == refs/heads/main' 'native-oracle production protected-main and full producer SHA authorization'
...
require_literal_count "$native_oracle_production_workflow" 'projection_schema:4,version:"0.7.36"' 2 'strict and survey RPM implementation pins'
require_job_match "$native_oracle_production_workflow" produce '\.schema_version == 3[\s\S]*\.artifact_type == "native-oracle-lane"[\s\S]*\.package_oracle\.schema_version == 1[\s\S]*\.resolution_oracle\.schema_version == 2[\s\S]*projection_schema:4[\s\S]*projection_schema:2' 'native-oracle production exact lane, package, resolution, and implementation schemas'
...
require_job_match "$resolution_survey_workflow" survey 'Download exact export and deployment evidence[\s\S]*remi-resolution-survey-transport\.py build-input[\s\S]*--assembly-evidence[\s\S]*--lane "fedora-44=[\s\S]*--lane "ubuntu-26\.04=[\s\S]*--lane "arch=[\s\S]*git merge-base --is-ancestor "\$deployed_commit" origin/main[\s\S]*git merge-base --is-ancestor "\$deployed_commit" "\$producer_commit"[\s\S]*git merge-base --is-ancestor "\$producer_commit" origin/main' 'resolution survey downloads and authenticates every exact assembled input'
...
require_job_match "$resolution_survey_workflow" survey 'git show "origin/main:deploy/remi-deploy-helper\.sh"[\s\S]*current_helper_sha256=[\s\S]*"\$helper_sha256" == "\$current_helper_sha256"' 'resolution survey refuses a historical protected-helper downgrade'
...
require_match "$resolution_survey_transport" 'validate_lane\([\s\S]*schema_version[^\n]*!= 3[\s\S]*native-oracle-lane[\s\S]*producer_commit[\s\S]*lane_evidence_sha256[\s\S]*authenticated three-lane assembly[\s\S]*resolution oracle is not bound to its package oracle' 'resolution survey validator exact assembled oracle lane bindings'
...
require_match "$resolution_survey_transport" 'read_tar_member\(archive, member, metadata\.st_size\)' 'resolution survey file admission derives from the bounded transport size'
...
require_match "$native_oracle_lane_producer" 'NATIVE_RESOLUTION_SURVEY_SCHEMA = 2[\s\S]*--survey[\s\S]*write_resolution_survey[\s\S]*native-resolution-survey-diagnostics[\s\S]*invoke\(resolution_command, "native resolution producer"\)' 'native-oracle lane writes and validates diagnostics survey before strict resolution'
