...
require_match "$candidate_deploy_workflow" 'completion_mode:[\s\S]*required: true[\s\S]*type: choice[\s\S]*private-candidates[\s\S]*active-repopulation' 'candidate deploy explicit typed completion mode'
