...
require_match "$site_deploy_workflow" 'concurrency:[\s\S]*group: deploy-and-verify[\s\S]*cancel-in-progress: false' 'site deployment serialized with production host authority'
require_match "$r2_durability_workflow" 'concurrency:[\s\S]*group: deploy-and-verify[\s\S]*cancel-in-progress: false' 'R2 durability serialized with production host authority'
require_match "$conversion_benchmark_workflow" 'workflow_dispatch:[\s\S]*deployment_run_id:[\s\S]*profile:[\s\S]*type: choice[\s\S]*fedora-44[\s\S]*ubuntu-26\.04[\s\S]*arch[\s\S]*profile_revision_sha256:[\s\S]*required: true[\s\S]*type: string[\s\S]*package_key_sha256:[\s\S]*source_url:[\s\S]*source_sha256:[\s\S]*source_size_bytes:[\s\S]*permissions:[\s\S]*actions: read[\s\S]*contents: read' 'conversion benchmark typed dispatch inputs and read-only GitHub permissions'
require_match "$conversion_benchmark_workflow" 'concurrency:[\s\S]*group: deploy-and-verify[\s\S]*cancel-in-progress: false' 'conversion benchmark serialized with deployment and verification'
require_job_match "$conversion_benchmark_workflow" benchmark 'timeout-minutes: 180[\s\S]*environment: production[\s\S]*ref: \$\{\{ github\.workflow_sha \}\}[\s\S]*GITHUB_REF" == refs/heads/main[\s\S]*git rev-parse HEAD\)" == "\$WORKFLOW_SHA"[\s\S]*git merge-base --is-ancestor "\$WORKFLOW_SHA" origin/main' 'conversion benchmark protected merged-main operator boundary'
require_job_match "$conversion_benchmark_workflow" benchmark 'actions/runs/\$\{DEPLOYMENT_RUN_ID\}[\s\S]*\.event == "workflow_dispatch"[\s\S]*\.status == "completed"[\s\S]*\.conclusion == "success"[\s\S]*\.head_branch == "main"[\s\S]*\.head_repository\.full_name == \$repository[\s\S]*deploy-remi-candidate\.yml[\s\S]*actions/download-artifact@3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c[\s\S]*remi-deployment-inspection-\$\{\{ inputs\.deployment_run_id \}\}' 'conversion benchmark exact successful protected deployment source'
require_job_match "$conversion_benchmark_workflow" benchmark 'deployment_evidence_schema_version == 3[\s\S]*completion_mode == "private-candidates"[\s\S]*repository_refreshes[\s\S]*latest_refresh\.finished_at[\s\S]*any\(\.candidates\[\]; \.profile == \$profile\)[\s\S]*\.source\.commit_sha == \$commit[\s\S]*\.source\.clean == true[\s\S]*\.build\.version \| type == "string"[\s\S]*\.build\.profile == "release"[\s\S]*\.build\.conary_git_commit == \$commit[\s\S]*\.build\.conary_git_dirty == "false"[\s\S]*\.artifact\.binary_sha256 == \$binary_sha256' 'conversion benchmark deployment inspection and clean candidate-manifest binding'
require_job_match "$conversion_benchmark_workflow" benchmark 'REQUESTED_REVISION: \$\{\{ inputs\.profile_revision_sha256 \}\}[\s\S]*\[\[ "\$REQUESTED_REVISION" =~ \^\[0-9a-f\]\{64\}\$ \]\][\s\S]*echo "revision=\$REQUESTED_REVISION"' 'conversion benchmark explicit comparable registered revision'
require_job_match "$conversion_benchmark_workflow" benchmark 'parsed\.scheme == "https"[\s\S]*parsed\.username is None[\s\S]*parsed\.password is None[\s\S]*--proto .?=https.[\s\S]*--proto-redir .?=https.[\s\S]*--max-filesize "\$source_size"[\s\S]*stat -c %s[\s\S]*sha256sum "\$source_path"' 'conversion benchmark credential-free bounded exact source acquisition'
require_job_match "$conversion_benchmark_workflow" benchmark 'REMI_SSH_KNOWN_HOSTS: \$\{\{ secrets\.REMI_SSH_KNOWN_HOSTS \}\}[\s\S]*\[\[ -n "\$\{REMI_SSH_KNOWN_HOSTS:-\}" \]\][\s\S]*printf .%s\\n. "\$REMI_SSH_KNOWN_HOSTS" > "\$known_hosts"[\s\S]*ssh-keygen -F "\$host" -f "\$known_hosts"[\s\S]*StrictHostKeyChecking=yes[\s\S]*UserKnownHostsFile="\$known_hosts"' 'conversion benchmark pinned production SSH host identity'
require_job_match "$conversion_benchmark_workflow" benchmark 'git show "\$\{WORKFLOW_SHA\}:deploy/remi-deploy-helper\.sh"[\s\S]*install-helper .\$helper_sha256. .\$remote_helper.[\s\S]*benchmark-remi-conversion .\$benchmark_id. .\$BINARY_SHA256. .\$PROFILE. .\$REVISION. .\$PACKAGE_KEY_SHA256. .\$SOURCE_SHA256. .\$SOURCE_SIZE_BYTES.[\s\S]*sha256sum "\$local_transport"[\s\S]*stat -c %s -- "\$local_transport"' 'conversion benchmark exact helper authority and authenticated public transport'
require_job_match "$conversion_benchmark_workflow" benchmark 'remote_source="/tmp/remi-conversion-source-\$\{benchmark_id\}\.native"[\s\S]*remote_transport="/tmp/remi-conversion-benchmark-\$\{benchmark_id\}\.json"[\s\S]*rm -f -- .\$remote_source. .\$remote_transport. .\$remote_helper.[\s\S]*"\$label" == "Conversion"[\s\S]*"\$operation" == "benchmark:"[\s\S]*"\$run_field" == "run=\$\{benchmark_id\}"[\s\S]*"\$transport_field" == "transport=\$\{remote_transport\}"' 'conversion benchmark fixed source and public transport lifecycle'
require_job_match "$conversion_benchmark_workflow" benchmark '\.raw_report\.schema_version == 3[\s\S]*\.environment\.source_commit == \$commit[\s\S]*\.environment\.binary_sha256 == \$binary_sha256[\s\S]*has\("binary_path"\) \| not[\s\S]*\.filesystem_type == "0x58465342"[\s\S]*\.authority\.selection_kind == "pinned"[\s\S]*map\(\.cache_state\)\) == \["cold", "hot"\][\s\S]*\.repetitions\[1\]\.views\.conversion_core\.executed == false[\s\S]*select\(\.\[-1\] == "path"[\s\S]*\.\[-1\] == "device_id"[\s\S]*\.\[-1\] == "error"' 'conversion benchmark public projection exact authority, cold-hot, XFS, and path-redaction validation'
require_job_match "$conversion_benchmark_workflow" benchmark 'def expected_roots: \[[\s\S]*"benchmark_cache",[\s\S]*"benchmark_chunks",[\s\S]*"benchmark_database",[\s\S]*"operator_source_artifact",[\s\S]*"repository_keys",[\s\S]*"source_catalogs",[\s\S]*"source_config",[\s\S]*"source_database",[\s\S]*"staged_source_artifact",[\s\S]*"work_root"[\s\S]*\];[\s\S]*\.environment\.roots \| map\(\.role\) \| sort\) == expected_roots[\s\S]*all\(\.environment\.roots\[\];[\s\S]*\.filesystem_type == "0x58465342"' 'conversion benchmark exact all-root XFS geometry'
require_job_match "$conversion_benchmark_workflow" benchmark 'def output_identity: \{\n[ ]{16}ccs_sha256,\n[ ]{16}ccs_size_bytes,\n[ ]{16}transport_sha256,\n[ ]{16}signed_object_set_sha256,\n[ ]{16}signed_object_count,\n[ ]{16}signed_object_bytes,\n[ ]{16}independent_transport_reopen_bytes,\n[ ]{16}independent_complete_archive_hash_bytes\n[ ]{14}\};[\s\S]*independent_transport_reopen_ms[\s\S]*type == "number" and \. >= 0 and floor == \.[\s\S]*independent_complete_archive_hash_ms[\s\S]*type == "number" and \. >= 0 and floor == \.[\s\S]*\.repetitions\[1\]\.output \| output_identity\)[\s\S]*== \(\.repetitions\[0\]\.output \| output_identity\)' 'conversion benchmark hot output identity and byte geometry exclude timing variance'
require_job_match "$conversion_benchmark_workflow" benchmark 'actions/upload-artifact@bbbca2ddaa5d8feaa63e36b76fdaad77386f024f[\s\S]*path: \|\n[ ]{12}remi-conversion-benchmark-public-v1\.json\n[ ]{12}remi-conversion-source-verification-v1\.json\n[ ]{12}remi-deployment-inspection\.json\n[ ]{12}remi-candidate-manifest\.json\n[ ]{10}if-no-files-found: error[\s\S]*compression-level: 0[\s\S]*retention-days: 30' 'conversion benchmark public-only retained evidence'
forbid_match "$conversion_benchmark_workflow" 'bash -s|/v1/admin|conversion-crawl|promotion-(prove|activate)|sudo -n (bash|sh)|rm -rf|conversion-benchmark-v3\.json|path:[^\n]*remi-conversion-source\.native' 'conversion benchmark generic authority or private/raw artifact upload'
forbid_match "$conversion_benchmark_workflow" 'ssh-keyscan|StrictHostKeyChecking=(accept-new|no)|UserKnownHostsFile=/dev/null' 'conversion benchmark live or weakened SSH host trust'
