...
require_job_match "$candidate_deploy_workflow" deploy-remi-candidate 'timeout-minutes: 180[\s\S]*inspect-remi"[\s\S]*> remi-predeployment-inspection\.json[\s\S]*transition_completed_at="\$\(date -u \+%s\)"[\s\S]*sleep 1[\s\S]*--max-time 7200 --request POST[\s\S]*http://127\.0\.0\.1:8081/v1/admin/refresh\?force=true[\s\S]*\.force == true[\s\S]*\.status == "complete" and \.failed == 0[\s\S]*\.status == "partial"[\s\S]*all\(\.failures\[\]; \.source_profile == "solus"\)' 'private candidate deploy forces a bounded post-transition public refresh'
