...
require_match "$candidate_postdeployment_filter" '\.repository_refreshes\[0\][\s\S]*\.scope == \{kind: "all"\}[\s\S]*\.finished_at > \$final\.deployment\.transition_completed_at[\s\S]*\.latest_refresh\.run_id == \.run_id[\s\S]*\.latest_refresh\.finished_at[\s\S]*> \$final\.deployment\.transition_completed_at\)\)[\s\S]*\.successful_profiles \| index\(\$profile\)[\s\S]*if same_fencing_authority\(\$before; \$final\) then[\s\S]*> fencing_epoch\(\$before; \$profile\)[\s\S]*else[\s\S]*fencing_epoch\(\$final; \$profile\) > 0' 'candidate deploy requires a typed post-transition refresh generation, candidate completion, and advances fences only within one schema authority'
