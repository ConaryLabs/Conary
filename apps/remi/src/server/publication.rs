...
/// Return whether a local CAS object is not reachable exclusively from stale
/// conversions.
///
/// Native publications are authoritative directly. Converted-package chunks
/// are hidden only when every referring conversion is stale; malformed current
/// metadata is surfaced by the model as a data error.
