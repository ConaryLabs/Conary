...
expected_scheme = sys.argv[5]
with public_key_path.open("rb") as stream:
    key = tomllib.load(stream)
...
try:
    ...
    if row is None:
        raise SystemExit("typed update repository was not persisted")
    repository_id = row[0]
    package = connection.execute(
        """
        SELECT name, version, version_scheme, architecture
        FROM repository_packages
        WHERE repository_id = ?
        """,
        (repository_id,),
    ).fetchall()
    expected_arch = "amd64" if expected_scheme == "debian" else "x86_64"
    expected = [
        (
    ...
    if package != expected:
        raise SystemExit(
            f"typed repository sync produced {package!r}, expected {expected!r}"
        )
    connection.execute(
        """
        UPDATE repositories
        SET default_strategy = 'static',
            source_profile = ?
        WHERE id = ?
        """,
    ...
    connection.execute(
        "DELETE FROM repository_package_keys WHERE repository_id = ?",
        (repository_id,),
    )
    connection.execute(
        """
        ...
        VALUES (?, ?, ?, 'active')
        """,
        (repository_id, key["key"], key.get("key_id")),
    )
    connection.commit()
finally:
    connection.close()
