...
use conary_core::generation::root_manifest::{
    CapturedSelectedRoot, materialize_captured_selected_root,
};
...
pub(super) fn apply_declarative_try_hooks(manifest: &CcsManifest, root: &Path) -> Result<()> {
    ...
    let mut executor = conary_core::ccs::HookExecutor::new(root, Default::default());
    executor
...
pub(super) fn promotable_try_hook_root(
    ...
    let artifact = conary_core::generation::artifact::load_generation_artifact(
        &runtime_root.generation_path(try_generation_id),
    )
    .with_context(|| {
        format!(
            "failed to load exact try generation artifact {}",
            try_generation_id
        )
    })?;
    ...
    if std::env::var_os("CONARY_TEST_SKIP_GENERATION_MOUNT").is_some() {
        ...
        if !root.is_dir() {
            bail!(
                "try generation {} has no materialized mutable-state upper at {}",
                try_generation_id,
                root.display()
            );
        }
