...
//! CCS package-authority resolution for repository-backed packages.
...
pub(super) fn resolve_ccs_package_authority(
    ...
    if default_strategy != Some("remi") {
        return load_explicit_package_authority(explicit_key_paths);
    }
    let endpoint = remi_endpoint.context("--remi-endpoint is required for Remi authority")?;
    let profile_id = profile_id.context("--source-profile is required for Remi authority")?;
    let canonical_keys =
    ...
    let keys = match (canonical_keys, explicit_key_paths.is_empty()) {
        ...
        (None, false) => load_explicit_package_authority(explicit_key_paths)?,
    };
...
fn load_explicit_package_authority(
    explicit_key_paths: &[PathBuf],
) -> Result<Vec<ResolvedCcsPackageKey>> {
    let keys = explicit_key_paths
        .iter()
        .map(|path| {
    ...
    for key in &keys {
        if !public_keys.insert(key.public_key.as_str()) {
            anyhow::bail!(
                "CCS package authority repeats public key {}",
                key.public_key
            );
        }
    }
