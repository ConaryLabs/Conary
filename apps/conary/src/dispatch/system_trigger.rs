...
pub(super) fn dispatch_system_trigger_command(trigger_cmd: cli::TriggerCommands) -> Result<()> {
    match trigger_cmd {
        cli::TriggerCommands::List { db, all } => commands::cmd_trigger_list(&db.db_path, all),

        cli::TriggerCommands::Show { name, db } => commands::cmd_trigger_show(&name, &db.db_path),
