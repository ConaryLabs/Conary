...
pub(super) fn dispatch_distro_command(distro_cmd: cli::DistroCommands) -> Result<()> {
    match distro_cmd {
        cli::DistroCommands::List { db } => commands::distro::cmd_distro_list(&db.db_path),
        cli::DistroCommands::Info { db } => commands::distro::cmd_distro_info(&db.db_path),
    }
}

pub(super) fn dispatch_canonical_command(can_cmd: cli::CanonicalCommands) -> Result<()> {
    match can_cmd {
...
pub(super) fn dispatch_groups_command(grp_cmd: cli::GroupsCommands) -> Result<()> {
    match grp_cmd {
...
pub(super) async fn dispatch_registry_command(reg_cmd: cli::RegistryCommands) -> Result<()> {
    match reg_cmd {
        ...
        cli::RegistryCommands::Stats { db } => commands::registry::cmd_registry_stats(&db.db_path),
    }
}
