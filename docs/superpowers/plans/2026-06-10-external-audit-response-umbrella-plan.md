...
| --- | --- | --- |
| Track 0: conaryd Authorization Fail-Closed Policy | `docs/superpowers/specs/2026-06-10-conaryd-authorization-fail-closed-policy-design.md` | `docs/superpowers/plans/2026-06-10-conaryd-authorization-fail-closed-implementation-plan.md` |
| Track 1: conary-test Evidence Trust | `docs/superpowers/specs/2026-06-10-conary-test-evidence-trust-design.md` | `docs/superpowers/plans/2026-06-10-conary-test-evidence-trust-plan.md` |
| Track 2: Release And CI Safety | `docs/superpowers/specs/2026-06-10-release-ci-safety-design.md` | `docs/superpowers/plans/2026-06-10-release-ci-safety-plan.md` |
| Track 3: Deploy And Artifact Integrity | `docs/superpowers/specs/2026-06-10-deploy-artifact-integrity-design.md` | `docs/superpowers/plans/2026-06-10-deploy-artifact-integrity-plan.md` |
| Track 4: Public Surface And Agent Routing Truth | `docs/superpowers/specs/2026-06-10-public-surface-agent-routing-truth-design.md` | `docs/superpowers/plans/2026-06-10-public-surface-agent-routing-truth-plan.md` |
| Track 5: Remaining Validator Soundness | `docs/superpowers/specs/2026-06-10-validator-soundness-follow-up-design.md` | `docs/superpowers/plans/2026-06-10-validator-soundness-follow-up-plan.md` |
...
| AUDIT-VALIDATOR-001 | Fable | Minor | partially-verified | Track 5 | Some validators still have fail-open edges such as header-only ledgers or shrinking scan scopes. | Plan after Tracks 1 and 2 unless a blocker appears. |
| AUDIT-ROUTE-DOC-001 | Gemini | Important | verified-by-structure | Track 5 | `scripts/check-doc-truth.sh` extracts conaryd routes only for separate `GET`, `POST`, and `DELETE` `.route(...)` calls and doc rows. | Promote to validator soundness when route surfaces change. |
| AUDIT-CLI-DOC-001 | DeepSeek | Important | verified-by-absence | Track 5 | `check-doc-truth.sh` has a conaryd route truth check but no analogous documented CLI command existence check for `conary` docs. | Consider after current coherency-ledger waves or in validator soundness. |
| AUDIT-LEGACY-REPLAY-001 | Fable | Minor | unverified | Track 4 | Legacy replay helpers may be duplicated across install/remove/system. | Verify before refactor; likely separate cleanup. |
| AUDIT-FLAG-001 | Fable | Minor | unverified | Track 4 | `experimental` feature flag may gate no production code. | Verify and remove or document. |
...
| AUDIT-DISPATCH-NAME-001 | DeepSeek | Minor | verified | Track 4 | `VerifyCommands` live in `cli/verify.rs` while dispatch lives in `dispatch/verify_derivation.rs`, which can confuse file-name based routing. | Add a routing comment or rename in a focused cleanup. |
