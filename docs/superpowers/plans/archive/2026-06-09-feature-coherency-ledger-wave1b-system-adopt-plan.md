...
> **Executed:** Wave 1b is closed in `docs/superpowers/feature-coherency-ledger.tsv` at commit `7b0f9396a98b31c30998fa87ff75bca18480b372`; do not rerun this plan. Treat embedded command and ledger snippets as planning-time snapshots and use the live files for current behavior.
...
- At planning time, docs-audit inventory count with this committed plan and the active single-package dry-run follow-up note was `172` tracked doc-like files. Refresh the count from `scripts/docs-audit-inventory.sh` instead of preserving this number.
- At planning time, the coherency ledger had four closed `1a-root-cli` rows and no `1b-system-adopt` rows. Wave 1b rows are now closed in the live ledger.
...
**Files:**
- Read: `docs/superpowers/plans/archive/2026-06-09-feature-coherency-ledger-wave1b-system-adopt-plan.md`
...
```bash
git ls-files docs/superpowers/plans/archive/2026-06-09-feature-coherency-ledger-wave1b-system-adopt-plan.md
...
```text
docs/superpowers/plans/archive/2026-06-09-feature-coherency-ledger-wave1b-system-adopt-plan.md
```
