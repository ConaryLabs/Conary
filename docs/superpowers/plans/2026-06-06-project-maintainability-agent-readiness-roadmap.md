...
- `test -z "$(git diff --no-index --check /dev/null docs/superpowers/plans/2026-06-06-project-maintainability-agent-readiness-roadmap.md)"`
  while the file is untracked, or `git diff --check` after it is staged/tracked.
- Placeholder and stale-surface sweep:
  `for term in T''BD TO''DO FI''XME Cent''OS RH''EL "Debian sta""ble" open''SUSE Al''pine CLAU''DE Cla''ude "Open Review"" Questions"; do rg -n "$term" docs/superpowers/plans/2026-06-06-project-maintainability-agent-readiness-roadmap.md; done`
