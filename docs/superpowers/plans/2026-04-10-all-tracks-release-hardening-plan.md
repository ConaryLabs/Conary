...
- if there is no operator-usable path without writing new code, record `signature rehearsal incomplete` in the checklist and mark the release `no-go` per the spec
...
- [ ] **Step 5: Watch the live GitHub workflows**

Run:

```bash
gh run list --workflow release-build.yml --limit 10
gh run list --workflow deploy-and-verify.yml --limit 10
```

Then watch the relevant live runs:
