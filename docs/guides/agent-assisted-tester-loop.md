...
Goal: validate the pinned Conary v0.9.0 package-manager preview loop and draft
...
- Use only the pinned v0.9.0 release from
...
```bash
mkdir -p "$HOME/conary-preview-v0.9.0"
cd "$HOME/conary-preview-v0.9.0"
```
...
```bash
base="https://github.com/ConaryLabs/Conary/releases/download/v0.9.0"
curl -fLO "$base/SHA256SUMS"
curl -fLO "$base/conary-0.9.0-1.fc43.x86_64.rpm"
```
...
base="https://github.com/ConaryLabs/Conary/releases/download/v0.9.0"
curl -fLO "$base/SHA256SUMS"
curl -fLO "$base/conary_0.9.0-1_amd64.deb"
```
...
base="https://github.com/ConaryLabs/Conary/releases/download/v0.9.0"
curl -fLO "$base/SHA256SUMS"
curl -fLO "$base/conary-0.9.0-1-x86_64.pkg.tar.zst"
```

Downloaded package names:

- Fedora 44: `conary-0.9.0-1.fc43.x86_64.rpm`
- Ubuntu 26.04 LTS: `conary_0.9.0-1_amd64.deb`
- Arch Linux: `conary-0.9.0-1-x86_64.pkg.tar.zst`

The Fedora package filename still carries the release-build RPM suffix
`fc43`; it is the pinned RPM asset for this Fedora 44 preview loop.
...
```bash
sudo dnf install ./conary-0.9.0-1.fc43.x86_64.rpm
```

Ubuntu 26.04 LTS:

```bash
sudo apt install ./conary_0.9.0-1_amd64.deb
```

Arch Linux:

```bash
sudo pacman -U ./conary-0.9.0-1-x86_64.pkg.tar.zst
```
