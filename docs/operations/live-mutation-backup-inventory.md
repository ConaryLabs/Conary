...
| `conary system generation publish --yes` | `AlwaysLive` | Replays exact persisted phases under the runtime mutation lock: select `/conary/current`, project configuration status, mark the matching state active, then write and verify `/conary/generations/<n>/state/conary.db.backup`, checksum, and manifest before publication debt becomes terminal. Link equality alone cannot complete debt. |
...
| `conary system restore` | `ActiveHostMutation` | Outside the current pre-alpha tester lane; touches live files and needs separate recovery evidence. |
...
before they enter the pre-alpha tester lane:
...
adoption quickstart. Keep them outside the pre-alpha tester lane until daemon
