...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `conary` | binary, `.ccs`, `.rpm`, `.deb`, `.pkg.tar.zst`, or source build | `.github/workflows/release-build.yml`, `scripts/release.sh conary`, `scripts/release-matrix.sh` | `v0.11.2` release candidate; exact tag commit pending publication | https://github.com/ConaryLabs/Conary/releases/tag/v0.11.2 | publication gate: record the exact tag commit and release-build/deploy-and-verify run IDs; checksums: independently verify every `SHA256SUMS` entry; verify the detached CCS signature; prove a native package initializes only its exact supported profile and can sync Remi; retain the W2 installed-binary self-update, rollback/redeploy, public-health, prewarm, and Fedora 44 `htop 3.4.1` clean-host baselines until the new artifact proof supersedes them; SBOM/provenance decision: not published, retained as an explicit limited-preview caveat; compatible rewritten Remi commit `27ec2eccb6befdf06d9a826b84cc5a6948eff5fb` and deployed pre-rewrite source commit `c001f8d69b9e8ef34fba39139576a9809800a9a6` have the same tree; deployed binary SHA-256 `c955a24ff6b90f98ba5f20b37e6a67b79bdde199ec0dcbfac0ce78b001d0f485` | release candidate; outreach gated | early preview; package installs can fail while Remi scriptlet review/adapters mature; native PM remains authoritative for adopted packages; Remi first-use conversion can be slow; no SBOM/provenance sidecars | `cargo build -p conary` remains available for source rebuilds |
