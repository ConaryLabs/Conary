...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `conary` | binary, `.ccs`, `.rpm`, `.deb`, `.pkg.tar.zst`, or source build | `.github/workflows/release-build.yml`, `scripts/release.sh conary`, `scripts/release-matrix.sh` | `v0.9.2` (`a4b81237e22939f6b01c48ea0823328fb5bbe279`) | https://github.com/ConaryLabs/Conary/releases/tag/v0.9.2 | checksums: `SHA256SUMS` verified locally for `.ccs`, `.rpm`, `.deb`, `.pkg.tar.zst`, and `metadata.json` on 2026-07-03; signature status: `conary-0.9.2.ccs.sig` verified with the configured self-update Ed25519 key on 2026-07-03; SBOM/provenance status: not published for `v0.9.2`; release-build `28626567664` green; deploy-and-verify `28628337460` green; release-matrix row; `conary --help` smoke; dated local KVM evidence for generation claims | limited preview | adoption-led only; native PM remains authoritative for adopted packages; Remi first-use conversion can be slow; SBOM/provenance sidecars are still pending | `cargo build -p conary` remains available for source rebuilds |
