...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `conary` | binary, `.ccs`, `.rpm`, `.deb`, `.pkg.tar.zst`, or source build | `.github/workflows/release-build.yml`, `scripts/release.sh conary`, `scripts/release-matrix.sh` | `v0.10.1` (`a1685e133e243764abe546ea0d4a3ce7ecd69ce1`) | https://github.com/ConaryLabs/Conary/releases/tag/v0.10.1 | checksums: `SHA256SUMS` published for `.ccs`, `.rpm`, `.deb`, `.pkg.tar.zst`, and `metadata.json`; signature status: `conary-0.10.1.ccs.sig` published and live self-update signature verification passed on 2026-07-07; SBOM/provenance status: not published for `v0.10.1`; release-build `28885333575` green; deploy-and-verify `28888132699` green; Remi latest endpoint reports `0.10.1`; `conary 0.10.0` self-updated to `0.10.1`; dated local KVM evidence for generation claims | limited preview | early preview; package installs can fail while Remi scriptlet review/adapters mature; native PM remains authoritative for adopted packages; Remi first-use conversion can be slow; SBOM/provenance sidecars are still pending | `cargo build -p conary` remains available for source rebuilds |
