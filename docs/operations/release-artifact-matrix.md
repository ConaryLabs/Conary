...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `conary` | binary, `.ccs`, `.rpm`, `.deb`, `.pkg.tar.zst`, or source build | `.github/workflows/release-build.yml`, `scripts/release.sh conary`, `scripts/release-matrix.sh` | `v0.11.1` at `4d4b422b45b055fa07a3885a68a4ab8e8d16b526` | https://github.com/ConaryLabs/Conary/releases/tag/v0.11.1 | checksums: verified independently against every `SHA256SUMS` entry; release-build `29540722051` and deploy-and-verify `29542934278` succeeded for the exact tag commit; the detached CCS signature verified against CCS SHA-256 `db1f4ef3c0c8654b0c49e5382635ef89b855bc0e89a69364ab341688073cda6f`; the official preceding-preview Fedora RPM binary verified the update signature, replaced itself with `v0.11.1`, and then reported itself current; the immutable failed predecessor tag produced no artifacts because release-build `29538737592` stopped on Rust 1.97 Clippy, and `v0.11.1` carries only that compatibility fix; SBOM/provenance decision: not published for `v0.11.1`, retained as an explicit limited-preview caveat; compatible Remi commit `c001f8d69b9e8ef34fba39139576a9809800a9a6`, exact deployed binary SHA-256 `c955a24ff6b90f98ba5f20b37e6a67b79bdde199ec0dcbfac0ce78b001d0f485`, public health 10/10, conversion-version-6 prewarm state 11 public and 1 private-review, successful rollback/redeploy rehearsal, and Fedora 44 clean-host `htop 3.4.1` install/run/remove | limited preview | early preview; package installs can fail while Remi scriptlet review/adapters mature; native PM remains authoritative for adopted packages; Remi first-use conversion can be slow; no SBOM/provenance sidecars | `cargo build -p conary` remains available for source rebuilds |
