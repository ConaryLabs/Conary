...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `conary` | binary, `.ccs`, `.rpm`, `.deb`, `.pkg.tar.zst`, or source build | `.github/workflows/release-build.yml`, `scripts/release.sh conary`, `scripts/release-matrix.sh` | immutable `v0.11.2`; annotated tag peels to `df8bb13dce759180081f38c6c78506e0a182fcd8` | https://github.com/ConaryLabs/Conary/releases/tag/v0.11.2 | release-build `29623998496` attempt 2 and deploy-and-verify `29626490327` passed at the exact tag commit; seven assets published; checksums independently matched all five `SHA256SUMS` payloads; detached CCS signature verified offline; released Arch package initialized only the Arch profile and synchronized 15,423 Arch Remi packages; official `v0.11.1` binary verified and applied `v0.11.2`, preserved schema 77, and reported itself current; full Remi health passed 10/10; compatible rewritten Remi commit `27ec2eccb6befdf06d9a826b84cc5a6948eff5fb` and deployed pre-rewrite source commit `c001f8d69b9e8ef34fba39139576a9809800a9a6` have the same tree; deployed public self-update API reports CCS SHA-256 `d00702da873192b4a5ad8836658f9ea8fe1dba1fa667bccdbd105561dfe5adc0`; checked `conary.io` and `remi.conary.io` production builds were deployed; SBOM/provenance decision: not published, retained as an explicit limited-preview caveat | limited preview verified; outreach remains gated by GitHub Support cached-history dereferencing and venue checks | early preview; package installs can fail while Remi scriptlet review/adapters mature; native PM remains authoritative for adopted packages; Remi first-use conversion can be slow; no SBOM/provenance sidecars | `cargo build -p conary` remains available for source rebuilds |
