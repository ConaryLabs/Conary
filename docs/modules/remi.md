...
authenticated alternative prevents this projection. Selections accumulate
across sibling required groups, and every apt-pkg negative relation on the
complete selected frontier is checked before a missing frontier is emitted;
conflicts or breaks between the root and a helper, between ancestors and
descendants, or between sibling helpers remain fatal native solver failures.
