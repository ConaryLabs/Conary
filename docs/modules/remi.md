...
helper, installs it through the existing `install-helper` action, and invokes
...
Once protected main advances the helper, rerunning an older workflow revision
fails before host contact; it cannot downgrade the root-owned entry point.
