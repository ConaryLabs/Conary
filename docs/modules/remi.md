...
conformance tests require complete typed projection without a runtime parser.

`native_only` admission returns `Admitted`,
`Excluded { class: NativeMachineArchitectureClassV1 }`, or
`UnknownArchitectureToken { scheme, token }`. Only a known non-native class
...
provider matching consumes the same checked source-derived classes. Every
package root has exactly one canonical resolved closure, typed unresolved set,
or known-class `architecture_excluded` outcome. Independent reopen uses a
