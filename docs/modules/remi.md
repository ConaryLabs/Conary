...
from the Fedora, Ubuntu, and Arch lane artifact names, derives the deployment
run from that export artifact, and requires the lane evidence, export manifest,
deployment inspection, installed binary digest, exact candidates, oracle
manifests, and typed architectures to agree. It transfers one authenticated
oracle archive and invokes the only production entry point:
...
`/conary/evidence/resolution-surveys/<survey-id>`, restores and probes Remi even
when the survey records findings, then emits
`/tmp/remi-resolution-survey-<survey-id>.tar`. The workflow independently
reopens that archive and uploads its canonical JSON, exact digest/size/binding
manifest, and count/histogram verification for seven days. This path cannot
promote, activate, or publish anything.
