...
residual hard required edges from the exact root or requiring packages still
eligible in the original strict solve after strict-priority exclusions. Edges
owned by a shadowed provider are validated but excluded from both the original
problem projection and the relaxed probe. That diagnostic
