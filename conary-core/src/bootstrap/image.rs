...
"#;
    ...
    ///
    /// Creates a cpio+gzip archive containing static busybox and an init script
    /// that mounts the root partition and execs systemd. The initramfs is installed
    /// to `{sysroot}/boot/initramfs.img`.
    ///
    ...
    pub fn generate_initramfs(&self, sources_dir: &Path) -> Result<PathBuf, ImageError> {
        ...
        // Find or obtain static busybox
        let busybox_cache = sources_dir.join("busybox-static");
        if !busybox_cache.exists() {
            // Try host system's busybox
            if let Ok(output_cmd) = Command::new("which").arg("busybox").output()
                && output_cmd.status.success()
            {
                let path = String::from_utf8_lossy(&output_cmd.stdout)
                    .trim()
                    .to_string();
                // Check if it's statically linked (ldd fails on static binaries)
                if let Ok(ldd_out) = Command::new("ldd").arg(&path).output()
                    && (!ldd_out.status.success()
                        || String::from_utf8_lossy(&ldd_out.stdout)
                            .contains("not a dynamic executable"))
                {
                    info!("Using host static busybox: {}", path);
                    fs::copy(&path, &busybox_cache)?;
                }
            }
            if !busybox_cache.exists() {
                return Err(ImageError::CommandFailed(
                    "Static busybox not found. Install busybox-static package \
                     or place a static busybox binary at sources/busybox-static"
                        .into(),
                ));
            }
        }
        ...
        #[cfg(unix)]
        for applet in ["sh", "mount", "switch_root", "sleep"] {
            let link = initramfs_root.join("bin").join(applet);
            if !link.exists() {
        ...
        // Create /dev/console device node (requires root, skip if not root)
        let console = initramfs_root.join("dev/console");
        if !console.exists() {
            let status = Command::new("mknod")
                ...
                .status();
            if let Err(e) = status {
                warn!("Could not create /dev/console (may need root): {}", e);
            }
        }
        ...
        let output_str = output
            .to_str()
            .ok_or_else(|| ImageError::CommandFailed("invalid output path".into()))?;
        let initramfs_str = initramfs_root
            .to_str()
            .ok_or_else(|| ImageError::CommandFailed("invalid initramfs path".into()))?;
